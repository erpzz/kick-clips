...
// Set a delay between pages in milliseconds
const DELAY_BETWEEN_PAGES_MS = 500; // Adjust this value as needed
...
// (Optional) If you don't need to map to a FeedItem structure, you can remove this function.
// Keeping it commented out if you want full raw JSON output with the appended score.
...
export async function fetchClipsForStreamer(streamer: string): Promise<ScoredKickClip[]> {
  ...
  while (pagesFetched < MAX_PAGES) {
    ...
    // Add a delay before the next pagination request
    console.log(`Waiting ${DELAY_BETWEEN_PAGES_MS}ms before fetching the next page...`);
